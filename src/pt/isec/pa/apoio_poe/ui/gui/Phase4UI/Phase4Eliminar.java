...
public class Phase4Eliminar extends BorderPane {
    Label lbProposta,lbAluno;
    public ComboBox<String> cbProposta,cbDocente;
    public Button btn1,btnVoltar;
    List<Propostas> p;
    List<Docente> d;
    public Phase4Eliminar(List<Propostas> p, List<Docente> d){
        this.d=d;
        this.p=p;
    ...
    private void registerHandlers() {
        cbProposta.setOnAction(actionEvent -> {
            for(Propostas propostas:p){
                if(propostas.getCodigoId()==cbProposta.getSelectionModel().getSelectedItem()){
                    cbDocente.getSelectionModel().select(propostas.getOrientador().getEmail());
                }
            }
        });
    }
