...
public class GestaoPropostasListarEstagio extends VBox {
    TableView tableView;
    List<Propostas> p;
    List<Estagio> est;
    public Button btnVoltar;
    public GestaoPropostasListarEstagio(List<Propostas> p){
        this.p=p;
        est = new ArrayList<>();
        for(Propostas prop:p){
            if(prop instanceof Estagio){
                est.add((Estagio) prop);
            }
        }
    ...
    private void createViews() {
        ...
        TableColumn empresa = new TableColumn("ORIENTADOR CONFIRMADO");
        empresa.setMinWidth(200);
        empresa.setMaxWidth(200);
        if(p!=null){
            ObservableList<Propostas> data = FXCollections.observableArrayList(est);
            ...
            empresa.setCellValueFactory(new PropertyValueFactory<Projeto,String>("empresa"));
            tableView.setItems(data);
        }

        tableView.getColumns().addAll(codigo,titulo,ramo,aluno,orientador,empresa);
