...
public class GestaoCandidaturaInserirNovo extends BorderPane {
    ...
    public GestaoCandidaturaInserirNovo(List<Aluno> a, List<Propostas> p,List<Candidatura> c){
        this.a=a;
        this.p=p;
        this.c=c;
        num = new ArrayList<>();
        for(Candidatura candidatura:c){
            a.remove(candidatura.getAluno());
        }
    ...
    private void update() {
        cbNumEstudante.setOnAction(actionEvent -> {
            ...
            selected.clear();
            Long nEstu = Long.parseLong(cbNumEstudante.getSelectionModel().getSelectedItem());
            SiglaRamo sr = null;
            for(Aluno aluno:a){
                if(aluno.getNumEstudante()==nEstu)sr=aluno.getSiglaRamo();
            }
            for(Propostas propostas:p){
                if(propostas.getRamo().contains(sr)){
                    ...
                    item0.setOnAction(actionEvent1 -> {
                        if(selected.contains(propostas.getCodigoId())){
                            selected.remove(propostas.getCodigoId());
                        }else{
